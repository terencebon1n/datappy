...
Both ingestors (trip update + alert) share one mock, so each successful cycle
records one ``run`` await per feed type.
"""
...
async def test_both_feed_types_are_ingested_each_cycle():
...
async def test_generic_exception_triggers_backoff_then_recovers():
    ...
    # iter2: both runs ok -> sleep(10) raises KeyboardInterrupt -> break.
    with _wired(
        run_side_effect=[RuntimeError("boom"), None, None],
        sleep_side_effect=[None, KeyboardInterrupt],
    ) as (kafka, admin, ingestor):
        await pm.ProducerService().start(City.MONTPELLIER)

    assert ingestor.run.await_count == 3
    kafka.stop.assert_awaited_once()
